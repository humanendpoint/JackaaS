...
class JamfUtils:
    ...
    def last_check_in(self, id):
        response = self.jamf.jamf_comm(f"{self.computerId}/{id}", method="GET", headers=self.json_get_headers)
        data = response.json()
    ...
    def get_all_computers(self):
        ...
        response = self.jamf.jamf_comm(self.computers, method="GET", headers=self.json_get_headers)
        json_data = response.json()
        return json_data

    def get_basic_info(self, id):
        response = self.jamf.jamf_comm(f"{self.computerId}/{id}/subset/General", headers=self.jamf.text_get_headers)
    ...
    def get_computer_details(self, id, category="general"):
        ...
        response = self.jamf.jamf_comm(f"{self.apiv1}/computers-inventory-detail/{id}", method="GET", headers=self.json_get_headers)
        data = response.json()
        ...
        if category in data:
            ...
            raise ValueError(f"Category '{category}' not found in the computer details.")

    # for now using classic API for this
    def get_specific_info(self, id):
        response = self.jamf.jamf_comm(f"{self.jss_api}/computers/id/{id}", method="GET", headers=self.jamf.json_get_headers)
        json_data = response.json()
        return json_data

    def get_computer_id_from_name(self, name, computers=None):
        if computers is None:
            computers = self.get_all_computers()["computers"]  # Use `computers` from API if not passed
    ...
    def get_files(self):
        response = self.jamf.jamf_comm(f"{self.apiv1}/jcds/files", method="GET", headers=self.json_get_headers)
    ...
    def get_file_link(self, filename):
        response = self.jamf.jamf_comm(f"{self.apiv1}/jcds/files/{filename}", method="GET", headers=self.json_get_headers)
        if response.status_code == 200:
            try:
                response_json = json.loads(response.text) 
    ...
    def generate_smart_group_chart(self, group_names, chart_type):
        ...
        for group_name in group_names:
            count = self.jamf.groups.count_computers_in_smart_group(group_name, create_missing=False)
        ...
        # Generate the chart
        chart_url = get_chart.get_current_chart(group_names, current_counts, chart_type, text="Group comparison on request")
    ...
    def get_computer_logs(self, id):
        response = self.jamf.jamf_comm(f"{self.jss_api}/computerhistory/id/{id}", method="GET", headers=self.json_get_headers)
        data = response.json()
        ...
        for log_entry in policy_logs:
            formatted_logs.append({
                ...
                "date_time": log_entry.get("date_completed", "Unknown Date"),  # Use 'date_completed' instead of 'date_time'
    ...
    def redeploy_framework(self, id):
        ...
        response = self.jamf.jamf_comm(url, method="POST", headers=self.json_get_headers)
        true_resp = response.json()
        return true_resp

    def get_recovery_key(self, id):
        response = self.jamf.jamf_comm(f"{self.apiv1}/computers-inventory/{id}/filevault", method="GET", headers=self.json_get_headers)
    ...
    def lockpass(self, id):
        response = self.jamf.jamf_comm(f"{self.apiv1}/computers-inventory/{id}/view-recovery-lock-password", method="GET", headers=self.json_get_headers)
    ...
    def devicelock(self, id, passcode):
        response = self.jamf.jamf_comm(f"{self.jss_api}/computercommands/command/DeviceLock/passcode/{passcode}/id/{id}", method="POST", headers=self.json_get_headers)
        if response == 201:
    ...
    def get_duplicates(self, duplicate_laptop):
        ...
        response = self.jamf.jamf_comm(url, method="GET", headers=self.jamf.text_get_headers)
        return response.text
    ...
    def oldest_newest(self, duplicate_computers):
        ...
        for comp in duplicate_computers:
            ...
            last_contact_time_dt = datetime.strptime(last_contact_time, "%Y-%m-%d %H:%M:%S") if last_contact_time != "N/A" else None
            last_enrolled_date_dt = datetime.strptime(last_enrolled_date, "%Y-%m-%dT%H:%M:%S.%f%z") if last_enrolled_date != "N/A" else None
    ...
    def get_appstore(self, computer_id):
        response = self.jamf.jamf_comm(f"{self.jss_api}/computerhistory/id/{computer_id}", method="GET", headers=self.json_get_headers)
        app_data = response.json()
        # Check if the mac_app_store_applications section exists
        if "computer_history" in app_data and "mac_app_store_applications" in app_data["computer_history"]:
    ...
    def mdm_expiry(self, id):
        ...
        if mdm_data:
            expiry_date = mdm_data.get("mdmProfileExpiration", "No MDM expiry date found.")
            return expiry_date
            
    def get_all_extattrs(self):
        response = self.jamf.jamf_comm(f"{self.apiv1}/computer-extension-attributes?page=0&page-size=100&sort=name.asc", method="GET", headers=self.jamf.text_get_headers)
