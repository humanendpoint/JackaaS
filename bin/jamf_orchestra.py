...
class JamfOrchestra:
    ...
    def orchestrate_checkin_all(self, jamf_computers, threshold_date, checkin_list):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_to_computer = {
                executor.submit(self.process_checkin, computer, threshold_date): computer
                for computer in jamf_computers["computers"] if "_" not in computer["name"]
            }
    ...
    def process_checkin(self, computer, threshold_date):
        ...
        try:
            if "_" not in computer["name"]:
                ...
                if checkin_info:
                    date_match = re.search(r"\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}", checkin_info)
                    if date_match:
                        last_contact_time_str = date_match.group()
                        last_contact_time = datetime.strptime(last_contact_time_str, "%Y-%m-%d %H:%M:%S")
    ...
    def orchestrate_get_computer_details(self, computer_names=None, computers=None, category="general"):
        if computer_names:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future_to_name = {
                    executor.submit(self.orchestrate_fetch_computer_details, name, computers, category): name for name in computer_names
                }
                computer_details = []
    ...
    def orchestrate_get_computer_processors(self):
        ...
        def get_processor(computer):
            id = computer["id"]
            processor = self.endpoint_details.get_computer_details(id, category="hardware")
            if processor:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = {
                executor.submit(get_processor, computer): computer for computer in all_computers["computers"]
            }
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    print(f"Error retrieving processor for computer {computer['id']}: {exc}")
    ...
    def orchestrate_get_computer_architectures(self):
        ...
        def get_architecture(computer):
            id = computer["id"]
            architecture = self.endpoint_details.get_computer_details(id, category="hardware")
            if architecture:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = {
                executor.submit(get_architecture, computer): computer for computer in all_computers["computers"]
            }
            for future in concurrent.futures.as_completed(futures):
                try:
                    result = future.result()
                    ...
                    computer = futures[future]
                    print(f"Error retrieving architecture for computer {computer['id']}: {exc}")
    ...
    def orchestrate_get_computer_models(self):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = {
                executor.submit(get_model, computer): computer for computer in all_computers["computers"]
            }
            for future in concurrent.futures.as_completed(futures):
                try:
                    result = future.result()
                    ...
                    computer = futures[future]
                    print(f"Error retrieving model for computer {computer['id']}: {exc}")
    ...
    def orchestrate_get_appstore_apps(self):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = {
                executor.submit(get_appstore, computer): computer for computer in all_computers["computers"]
            }
            for future in concurrent.futures.as_completed(futures):
                try:
                    result = future.result()
                    ...
                    computer = futures[future]
                    print(f"Error retrieving appstore for computer {computer['id']}: {exc}")

        installed_apps = []
        excluded_apps = ["Pages.app", "GarageBand.app", "iMovie.app", "Keynote.app", "Numbers.app"]
    ...
    def orchestrate_get_appstore(self, computer_id):
        ...
        if "pending" in appstore_apps:
            for app_info in appstore_apps["pending"]:
                ...
                pending_apps.append(f"🕒 `{app_name}` (v{app_version}) - Deployed: {deployed}")
        ...
        if "failed" in appstore_apps:
            for app_info in appstore_apps["failed"]:
                ...
                failed_apps.append(f"❌ `{app_name}` (v{app_version}) - Status: {status}")
    ...
    def orchestrate_get_appstore_overview(self, number):
        ...
        if top_apps:
            ...
            for rank, (app_name, count) in enumerate(top_apps, start=1):
                pretty_output.append(f"{rank}. `{app_name}`: Installed on `{count}` computers")
            pretty_output.append(f"\n*Total unique apps installed*: `{counted_apps}`\n*Excluding*: {", ".join(excluded_apps)}")
            return "\n".join(pretty_output)
        else:
            return "No installed apps data available."
    
    def orchestrate_mdm_commandhistory(self, computer_id):
        completed, pending, failed = self.endpoint_details.get_computerhistory(computer_id)
        # base payload
        payload = {
            "blocks": [
                {"type": "section", "text": {"type": "mrkdwn", "text": "*Commands history:*"}},
            ]
        }
        ...
        def create_command_blocks(title, commands, fields):
            ...
            for command in commands:
                section_blocks.append(
                    {
                        "type": "section",
                        "fields": [{"type": "mrkdwn", "text": "\n".join(f"*{key}:* `{command[val]}`" for key, val in fields.items())}]
                    }
                )
            return section_blocks
        # structure for completed, pending, and failed
        if completed:
            payload["blocks"].extend(create_command_blocks(
                "Completed",
                completed,
                {"Command": "name", "Date": "completed"}
            ))
        if pending:
            payload["blocks"].extend(create_command_blocks(
                "Pending",
    ...
    def orchestrate_mdm_expiry(self, threshold_date):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = {
                executor.submit(get_expiry, computer): computer for computer in all_computers["computers"] if "_" not in computer["name"]
            }
            for future in concurrent.futures.as_completed(futures):
                try:
                    result = future.result()
                    if result:
                        ...
                        print(f"computer name: {computer_name} mdm_expiry_str: {mdm_expiry_str}")
                        # Remove 'Z' and convert to naive datetime
                        mdm_expiry_time = datetime.strptime(mdm_expiry_str.replace("Z", ""), "%Y-%m-%dT%H:%M:%S")
                    ...
                    computer = futures[future]
                    print(f"Error retrieving MDM expiry for computer {computer['id']}: {exc}")
    ...
    def orchestrate_duplicates(self, duplicate_laptops):
        ...
        for laptop in duplicate_laptops:
            ...
            if xml_response:
                try:
                    ...
                    computers = [
                        {"id": int(computer.find("id").text), "name": computer.find("name").text}
        ...
        for computer in all_computers:
            ...
            try:
                ...
                serial_number = serial_number.text # Assuming serial_number is directly under the root
                computer["serial_number"] = serial_number  # Add serial number to the computer record
            except XML.ParseError as e:
        ...
        duplicate_computers = {name: comps for name, comps in name_to_computers.items() if len(comps) > 1}
        ...
        payload = {
            "blocks": [
                {"type": "section", "text": {"type": "mrkdwn", "text": "*Duplicates:*"}},
                {"type": "divider"},
            ]
        }
    ...
    def process_reboots(self, computer, threshold_date):
        ...
        name = computer["name"]
        details = self.orchestrate_get_computer_details(computer_names=[name], computers=[computer], category="hardware")
        if details:
            ext_attributes = details[0].get("extensionAttributes", [])
            last_startup = next((attr for attr in ext_attributes if attr['definitionId'] == '29'), None)
            if last_startup and last_startup.get('values'):
                last_startup_date_str = last_startup['values'][0]
                try:
                    last_startup_date = datetime.strptime(last_startup_date_str, "%Y-%m-%d %H:%M:%S")
    ...
    def orchestrate_reboots(self, args, threshold_date, startup_data):
        ...
        if reboots[0].lower() == "all":
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future_to_computer = {
                    executor.submit(self.process_reboots, computer, threshold_date): computer for computer in computers["computers"] if "_" not in computer["name"]
                }
                for future in concurrent.futures.as_completed(future_to_computer):
                    computer = future_to_computer[future]
                    try:
        ...
        # Check if specific user(s) are provided
            ...
            details = self.orchestrate_get_computer_details(computer_names=computer_names, category="hardware")
            if details:
                for idx, detail in enumerate(details):
                    ext_attributes = detail.get("extensionAttributes", [])
                    last_startup = next((attr for attr in ext_attributes if attr['definitionId'] == '29'), None)
                    if last_startup:
                        last_startup_date_str = last_startup['values'][0]  # Extract date string
                        last_startup_date = datetime.strptime(last_startup_date_str, "%Y-%m-%d %H:%M:%S")
                        startup_data.append(f"`{computer_names[idx]}`: {last_startup_date}")  # Reference name correctly
                    else:
                        startup_data.append(f"No startup data found for `{computer_names[idx]}`.")
