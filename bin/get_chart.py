...
def get_current_chart(labels, current_counts, chart_type, text):
    ...
    if len(labels) > len(segment_colors):
        ...
        additional_colors = [random_color() for _ in range(len(labels) - len(segment_colors))]
    ...
    if chart_type == "pie" or chart_type == "doughnut":
        cutout_percentage = 50 if chart_type == "doughnut" else 0  # default for doughnut
        datasets = [{
            "data": current_counts,
        ...
        datasets = [{
            "label": "Current",
            "data": current_counts,
            "backgroundColor": QuickChartFunction("getGradientFillHelper('vertical', ['#00FF00', '#FFA500', '#FF0000'])")
        }]
        options = {
            "title": {
                "display": True,
