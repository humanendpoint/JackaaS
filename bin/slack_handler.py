...
class SlackHandler:
    ...
    def handle_message(self, message, say, ack: Ack):
        ...
        authorized, cmd_key = self.user_auth.is_user_authorized(user_id, text, response, self.app.client)
        ...
        if cmd_key in self.commands:
            ...
            self.app.client.chat_update(
                channel=response['channel'],
                ts=response['ts'],
                text="Unknown command. Please use one of the following: " + ", ".join(self.commands.keys())
            )
    ...
    def handle_help(self, *args):
        ...
        command_list = "\n".join([f"`{cmd}`: {desc}" for cmd, desc in self.help.items()])
    ...
    def handle_count_group(self, args):
        ...
        if len(count_cmd) >= 2:
            group_name = " ".join(count_cmd[0:-1])  # Join everything except the last part as group name
            create_missing = count_cmd[-1].lower() == "true"  # The last part is the true/false flag
    ...
    def handle_details(self, args):
        ...
        if len(deets) >= 2:
            ...
            try:
                details = self.jamf_client.orchestra.orchestrate_get_computer_details(computer_names=computer_names, category=category)
                if details:
                    details_str = "\n".join(
                        "\n".join(f"{key}: {value}" for key, value in detail.items()) if isinstance(detail, dict) else str(detail)
    ...
    def handle_create_group(self, args):
        ...
        if len(parts) >= 2:
            ...
            and_or = parts[2].lower() if len(parts) > 2 and parts[2].lower() in ["and", "or"] else "and"
            computers = parts[3].split() if len(parts) > 3 else None
            if computers:
                self.groups.create_group(group_name, is_smart="false", computers=computers)
                return f"Static group `{group_name}` has been created with the specified computers."
            else:
                self.groups.create_group(group_name, criterion_name, is_smart="true", and_or=and_or)
    ...
    def handle_count_computers(self, args):
        ...
        if len(parts) >= 3:  # At least category, subset, and a value
            ...
            count = self.jamf_client.orchestra.orchestrate_count_computers_subset(category, subset, value)
    ...
    def chart_helper(self, data_source_method, chart_type="horizontalBar", chart_text="Comparison on request", counts=None):
        ...
        # Generate the chart with counts and labels
        chart_url = self.jamf_utils.generate_other_chart(labels, counts, chart_type, text=chart_text)
        return chart_url
    ...
    def handle_show_script(self, args):
        ...
        if len(script) >= 1:
            if script[0] == "all":
                ...
                script_content = self.jamf_client.scripts.get_script_by_name(script_name)
        ...
        if len(mdm_command_log) >= 1:
            for computer_name in mdm_command_log:
                if "." in computer_name or "CW" in computer_name:
                    computer_id = self.jamf_utils.get_computer_id_from_name(computer_name, computers=None)
                mdm_command_log_info = self.jamf_client.orchestra.orchestrate_mdm_commandhistory(computer_id)
    ...
    def handle_appstore(self, args):
        ...
        for computer_name in appstoreapps:
            if "." in computer_name or "CW" in computer_name:
                computer_id = self.jamf_utils.get_computer_id_from_name(computer_name, computers=None)
                appstore_info = self.jamf_client.orchestra.orchestrate_get_appstore(str(computer_id))
                if appstore_info:
    ...
    def handle_mdmexpiry(self, args):
        ...
        if len(expiry) >= 1:
            if expiry[0] == "all":
                ...
                expiry_dates = self.jamf_client.orchestra.orchestrate_mdm_expiry(threshold_date)
                return expiry_dates
    ...
    def handle_checkin(self, args):
        ...
        if len(checkin) >= 1:
            if checkin[0] == "all":
                ...
                checkins = self.jamf_client.orchestra.orchestrate_checkin_all(jamf_computers, threshold_date, checkin_list)
                return checkins
            else:
        ...
        if len(log) >= 1:
            for computer_name in log:
                ...
                if computer_id is not None:
                    log_info = self.jamf_client.orchestra.orchestrate_get_computer_logs([computer_id])
                    if log_info:
                        # Formatting output for better readability
                        formatted_log_info = "\n".join(f"{entry['policy_name']} *Date run*: {entry['date_time']} *Status*: {entry['status']}" for entry in log_info)
        ...
        if len(recovery) >= 1:
            for computer_name in recovery:
                if "." in computer_name or "CW" in computer_name:
                    computer_id = self.jamf_utils.get_computer_id_from_name(computer_name, computers=None)
                if computer_id is not None:
                    recovery_info = self.jamf_client.orchestra.orchestrate_recoverykey(computer_id)
                    if recovery_info:
        ...
        if len(redeploy) >= 1:
            for computer_name in redeploy:
                if "." in computer_name or "CW" in computer_name:
                    computer_id = self.jamf_utils.get_computer_id_from_name(computer_name, computers=None)
                if computer_id is not None:
                    redeploy_info = self.jamf_client.orchestra.orchestrate_redeploy(computer_id)
                    if redeploy_info:
    ...
    def handle_devicelock(self, args):
        ...
        if len(lock) >= 2:
            ...
            if "." in computer or "CW" in computer:
                computer_id = self.jamf_utils.get_computer_id_from_name(computer, computers=None)
            if computer_id is not None:
    ...
    def handle_reboots(self, args):
        ...
        days_threshold = 60
        threshold_date = datetime.now() - timedelta(days=days_threshold)
        startup_data = self.jamf_client.orchestra.orchestrate_reboots(args, threshold_date, startup_data)
        ...
        if startup_data:
            ...
            return "All specified computers have started within the last specified days."
    ...
    def handle_membership(self, args):
        ...
        if len(users) >= 1:
            output = []
            for user in users:
                membership_info = self.jamf_client.orchestra.orchestrate_get_computer_details(
                    computer_names=[user], category="groupMemberships"
                )
                if membership_info:
                    # since membership_info is a list, iterate over it to access group details
                    group_names = []
                    for group in membership_info[0]:  # assuming membership_info[0] is a list of groups
                        group_names.append(group["groupName"])
                    if group_names:
                        output.append(f"Membership info for `{user}`:\n```" + "\n".join(group_names) + "```")
                    else:
    ...
    def count_computers_in_group(self, group_name, create_missing):
        ...
        count = self.groups.count_computers_in_smart_group(group_name, create_missing=create_missing)
